    ...
    try {
        ...
        for (const transaction of transactions) {
            try {
                ...
                //let result = ""
                const axiosGetResp = await axios.get(transaction.aggregatedResults["speechToText"].location, axiosParams)
                if (axiosGetResp?.data?.status) {
                    status = axiosGetResp.data.status
                } else {
                    throw new Error(`failed in Speech accessing ${transaction.aggregatedResults["speechToText"].location}`)
                }
                if (status === 'Failed') {
                    throw new Error(`batch transcription failed ${transaction.aggregatedResults["speechToText"].filename}`)
                }
